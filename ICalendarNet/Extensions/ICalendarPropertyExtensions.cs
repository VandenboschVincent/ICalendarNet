...
namespace ICalendarNet.Extensions
{
    public static class ICalendarPropertyExtensions
    {
        ...
        {
            var foundLine = lines.Find(t => t.Name.Equals(key, StringComparison.OrdinalIgnoreCase));
            if (foundLine != null)
            {
                if (parameters != null)
                    foundLine.Parameters = parameters;
            ...
            lines.Add(GetPropertyType(key).GetContentLine(key, value, parameters));
        }
        ...
        internal static void UpdateLinesProperty(this List<ICalendarProperty> lines, IEnumerable<string> value, string key, ContentLineParameters? parameters = null)
        {
            lines.RemoveAll(t => t.Name.Equals(key, StringComparison.OrdinalIgnoreCase));
            lines.AddRange(value.Select(t => GetPropertyType(key).GetContentLine(key, t, parameters)));
        }

        internal static bool IsNewProperty(this string line)
        {
            if (string.IsNullOrWhiteSpace(line))
                return false;
            string _line = line.Trim();
            if (_line.StartsWith("X-") || Array.Exists(ICalProperties, _line.StartsWith))
                return true;
            return false;
        }
        ...
        {
            int index = ICalProperties.AsSpan().IndexOfAny(key);
            if (index >= 0)
                return property;
            if (key[0] != 'X')
                throw new InvalidDataException($"{key} is not a recognized property type.");
            return null;
        }
